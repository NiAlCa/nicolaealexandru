export const CurrentlyWorkData = [
  {
    image: "/src/assets/SomeProjects/coda.png",
    tittle: "Coda",
    text: "Website focused on musical NFTs for which I have been fully responsible for the front-end development.",
    url: "https://coda.to/",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/rusty.png",
    tittle: "Rusty robot country club",
    text: "NFT project website on which I have worked on different parts such as the section where you have the comic gallery and a swiper to read them, part of the music section, and the governance requests part.",
    url: "https://www.rustyrobot.io/",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/fichaje.png",
    tittle: "Work Time Recorder",
    text: "Developed a React Native application for recording work time, leveraging JavaScript for a seamless user experience and efficiency in tracking hours",
    url: "https://github.com/NiAlCa/WorkTimeRecorder-ReactNative",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/ping.png",
    tittle: "Ping monitor",
    text: "PingMonitor is a sleek desktop application designed to track and display real-time ping responses from various locations, ensuring seamless network connectivity and operational efficiency.",
    url: "https://github.com/NiAlCa/PingElectron",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/pdf.png",
    tittle: "PDF Organizer",
    text: "The PDF Processor Efficiently manage your documents by selecting keywords and directories. Boost productivity today with our user-friendly tool",
    url: "https://github.com/NiAlCa/Order-pdfs",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/mercadona.png",
    tittle: "MTR Mercadona to excel",
    text: "Executable script that reads MTR PDFs from Mercadona and generates an Excel table to organize the warehouse, saving time and avoiding human error.",
    url: "https://github.com/NiAlCa/mercadonaMTR-pdf-script",
    id: "Work",
  },
  {
    image: "/src/assets/SomeProjects/openai.png",
    tittle: "ChatBot with OpenAI API",
    text: "A chatbot developed using the OpenAI API represents an advanced automatic interaction tool designed to simulate human conversations in an intelligent and coherent way.",
    url: "https://github.com/NiAlCa/ChatBot",
    id: "Project",
  },

  {
    image: "/src/assets/SomeProjects/apuntomotor.png",
    tittle: "Apuntomotor.com",
    text: "The Landing Page of the 'ApuntoMotor' Business, showcasing their automotive services and products.",
    url: "https://apuntomotor.com/",
    id: "Work",
  },

  {
    image: "/src/assets/SomeProjects/expensiveTracker.png",
    tittle: "Expensive Tracker project",
    text: "Project carried out to reinforce and improve my knowledge using Context.",
    url: "https://github.com/NiAlCa/expensiveTracker",
    id: "Project",
  },
  {
    image: "/src/assets/SomeProjects/pokedex.png",
    tittle: "Pokedex project",
    text: "Project carried out to reinforce and improve my knowledge using APIs.",
    url: "https://github.com/NiAlCa/Pokedex",
    id: "Project",
  },

  {
    image: "/src/assets/SomeProjects/anthrboy.png",
    tittle: "Anthrboy.com",
    text: "The website of the artist Anthrboy, featuring a portfolio of his digital art, music projects, and creative endeavors.",
    url: "https://anthrboy.com/",
    id: "Work",
  },

  {
    image: "/src/assets/SomeProjects/ea.png",
    tittle: "Develhope team project",
    text: "Project consisting of cloning the EA website to put into practice what has been learned. JS, TS, React and more",
    url: "https://github.com/develhope-e4/ea-sports",
    id: "Project",
  },

  {
    image: "/src/assets/SomeProjects/rolling.png",
    tittle: "Develhope team project",
    text: "Project consisting of cloning the RollingStones website to put into practice what has been learned. Html, Css, SaSS and more",
    url: "https://github.com/IluHeart/PAGINA-WEB-PROYECTO",
    id: "Project",
  },

  {
    image: "/src/assets/SomeProjects/emmaembaladora.png",
    tittle: "Emmaembaladora.com",
    text: "This website, created using WordPress, highlights the innovative manual packaging device.",
    url: "https://emmaembaladora.com/",
    id: "Work",
  },

  {
    image: "/src/assets/SomeProjects/tecmetal.png",
    tittle: "Tecmetal.es",
    text: "Presenting 'Tecmetal', a metal carpentry business, through a website I developed as a programmer using WordPress.",
    url: "https://tecmetal.es/",
    id: "Work",
  },

  {
    image: "/src/assets/SomeProjects/apuntomotor.png",
    tittle: "Apuntomotor.com",
    text: "The Landing Page of the 'ApuntoMotor' Business, my first webpage with wordpress.",
    url: "https://apuntomotor.com/",
    id: "Work",
  },
];
